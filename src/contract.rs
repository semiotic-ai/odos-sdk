...
pub const ODOS_V2_UNICHAIN_ROUTER: &str = "0x6409722F3a1C4486A3b1FE566cBDd5e9D946A1f3";
...
pub const ODOS_V3: &str = "0x0D05a7D3448512B78fa8A9e46c4872C88C4a0D05";
...
/// ```
pub fn get_v2_router_by_chain_id(chain_id: u64) -> Option<&'static str> {
...
/// }
/// ```
pub fn get_v3_router_by_chain_id(chain_id: u64) -> Option<&'static str> {
    let named_chain = NamedChain::try_from(chain_id).ok()?;
...
/// }
/// ```
pub fn get_both_router_addresses(chain_id: u64) -> Option<(&'static str, &'static str)> {
    match (
    ...
    fn test_all_addresses_are_valid_hex() {
        ...
        for address in addresses {
            ...
            assert_eq!(
                address.len(),
                42,
                "Address should be 42 characters long: {address}",
            );
            assert!(
        ...
        for (chain_id, named_chain) in test_chains {
            ...
            if let Some(v2_addr_str) = get_v2_router_by_chain_id(chain_id) {
                if let Ok(v2_addr_trait) = named_chain.v2_router_address() {
                    let v2_addr_util: Address = v2_addr_str.parse().unwrap();
                    assert_eq!(v2_addr_trait, v2_addr_util);
                }
            }

            if let Some(v3_addr_str) = get_v3_router_by_chain_id(chain_id) {
                if let Ok(v3_addr_trait) = named_chain.v3_router_address() {
                    let v3_addr_util: Address = v3_addr_str.parse().unwrap();
                    assert_eq!(v3_addr_trait, v3_addr_util);
                }
            }
        }
    }
